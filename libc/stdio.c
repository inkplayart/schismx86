...
int putchar(int character) //yes, it's putchar,not putc, which is a different function
{
	...
	systemCall(PUTC_CALL,stdout); //this sys call just needs the output stream
	return 1; //anything but EOF
}
...
int getchar()
{
	...
	if(stdin->filePos == (long unsigned int)stdin->bufferSize-1)
		return 0; //fail
	
	//Now we have to do a system call, whose sole purpose is to kernel_getch. This means we need to send in the file
	systemCall(GETCHAR_CALL,stdin);
	
	//Now it's in stdin, we need to get rid of it
	int retVal = stdin->buf[stdin->filePos];
	if(stdin->filePos > 0)
		stdin->filePos--; //For stdin, we actually do reduce the filepos, unless it's zero. If it's zero there is only one thing in there
	return retVal;
}